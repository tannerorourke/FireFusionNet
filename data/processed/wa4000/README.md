...
Daily spatio-temporal datacube for **wildfire ignition and cause prediction** over Washington State, 2003-2020, at 4 km resolution. Supervision is restricted to the **May-October fire season**, the months that carry Washington's fire activity.
...
Extraction is deliberately wider than what ships. Each year is pulled as a single block running **March 22 to November 10**, the fire season plus a 40-day lead and a 10-day trail, so 234 days are extracted per year against the 184 that are supervised. Over 2003-2020 that is 4212 days extracted and 3312 published.
...
| --- | --- | --- | --- |
| train | 2003-2016 | 2576 | 2.4 GB |
| eval | 2017-2018 | 368 | 347 MB |
| test | 2019-2020 | 368 | 353 MB |
...
| --- | --- | --- |
| 0 | `NATURAL_LIGHTNING` | 14917 |
| 1 | `HUMAN` | 5781 |
| 2 | `INDUSTRIAL` (includes debris) | 780 |
...
Ignition is heavily imbalanced: **`ign_pos_weight` = 973.95** on the train split, so positives are ~1.0e-3 of supervised cell-days. Restricting to the fire season removes winter cell-days that are near-uniformly negative, so this is a fire-season base rate and not an annual one. Losses should apply `land_mask` and `no_act_fire_mask`; the cause head should additionally apply `valid_cause_mask`.
...
Streaming directly from the Hub works for inspection, but **not for training**: `X` chunks are `(16, 38, 102, 109)`, about 27 MB each decompressed and ~15 MB on the wire. A 10-day window spans one or two of them, so a train epoch of ~2450 windows moves on the order of 56 GB, roughly 23x the cost of downloading the 2.4 GB split once. Download, then train.
...
| --- | --- | --- |
| **wa4000** (this one) | 4000 m | 102 x 109 |
| [wa2000](https://huggingface.co/datasets/torq1/fire-fusion-wa-2000m) | 2000 m | 204 x 217 |
